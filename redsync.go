...
// redissync provides a simple method for creating distributed mutexes using multiple Redis connection pools.
type redissync struct {
	pools []Pool
}

// New creates and returns a new redissync instance from given Redis connection pools.
func New(pools []Pool) *redissync {
	return &redissync{
		pools: pools,
	}
}
...
func (r *redissync) NewMutex(name string, options ...Option) *Mutex {
	m := &Mutex{
		name:   name,
